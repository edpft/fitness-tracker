//! Raw landing for `garmin.activity_files`.
//!
//! **A copy of [`super::garmin_exercise_set_landing`] rather than a generalisation
...
/// The landing table for Garmin's activity files: each activity's FIT
/// recording, in the archive Garmin serves it in.
///
/// **Another table behind the same catalogue entry**, as Peloton's graphs sit
/// behind its rides, and the third after the list and its exercise sets.
/// A landing record holds one response as served (§ II.1).
#[derive(Debug, Clone)]
pub struct GarminActivityFileLandingStore {
    pool: SqlitePool,
    stream: LandingStream,
}

impl GarminActivityFileLandingStore {
    /// Which stream this table holds.
    ///
    /// Declared beside the queries that name `garmin_activity_file_landing`, for the
    /// reason given on [`super::landing::HevyWorkoutLandingStore::STREAM`]:
    /// this is the one link no type can check.
    pub const STREAM: &'static str = "garmin.activity_files";
...
impl LandingStore for GarminActivityFileLandingStore {
    ...
    async fn append(
        ...
        for record in &records {
            ...
            sqlx::query!(
                r#"
                INSERT INTO garmin_activity_file_landing (
    ...
    async fn count(&self) -> Result<RecordCount, StoreError> {
        let row =
            sqlx::query!(r#"SELECT COUNT(*) AS "total!: i64" FROM garmin_activity_file_landing"#)
                .fetch_one(&self.pool)
                .await
                .map_err(|error| store_error(&error))?;
...
#[cfg(test)]
mod tests {
    use super::{GarminActivityFileLandingStore, LandingStream};

    /// The constant every run's identity is derived from must name a stream.
    #[test]
    fn the_declared_stream_is_a_stream() {
        let stream =
            LandingStream::try_from(GarminActivityFileLandingStore::STREAM).expect("a stream name");
        assert_eq!(stream.to_string(), "garmin.activity_files");
    }
}
