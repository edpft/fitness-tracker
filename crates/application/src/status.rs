...
use crate::{
    error::StatusError,
    ports::{
        ExtractionRunLog, LandingStore, ReportExtractionStatus, ResetResumptionPoint,
...
impl<L, R, G> ReportExtractionStatus for ExtractionStatus<L, R, G>
where
    L: LandingStore + Sync,
    R: ResumptionPointStore + Sync,
    G: ExtractionRunLog + Sync,
{
    async fn status(&self, stream: &LandingStream) -> Result<StreamStatus, StatusError> {
...
impl<L, R, G> ResetResumptionPoint for ExtractionStatus<L, R, G>
where
    L: LandingStore + Sync,
    R: ResumptionPointStore + Sync,
    G: ExtractionRunLog + Sync,
{
    async fn reset(&self, stream: &LandingStream) -> Result<(), StatusError> {
        ...
        self.resumption.clear(stream).await?;
        Ok(())
    }
}
