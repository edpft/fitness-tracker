...
fitness normalise hevy.workouts --timezone Europe/London
...
| exercise entries | 1,135 of 1,135 |
| sets translated | 3,778 of 3,779 |
| supersets | 334 of 336 groupings, all the well-formed ones |
...
That is what distinguishes an absorbing retraction from latest-wins.
...
## Scenario 4 — The mapping reads each exercise's load axis the right way

*The mapping's specification.*

A squat and a deadlift are absolute: the number is external load and none of it
is a real answer. A pull-up and a dip are relative: assistance is conventional,
so the axis runs through zero and the sign carries meaning. Assert the four, and
assert that the pull-up family's three templates and the chest dip's two each
reach one exercise.

Then assert the collapses that are the same collapse: a weighted crunch and a
plain one, a weighted hyperextension and an unweighted one.
...
| `NonContiguousGrouping` | wrong data | 1 |
| `SingleMemberGrouping` | wrong data | 1 |
| anything else | — | 0 |

One set and two groupings, and **no refusal outside that set**. Each names the
landing record, the position within it, the exercise where one was resolved, and
a reason that distinguishes the three kinds — asserted as a query over
`RefusalReason`, never over rendered text.
...
- 2,414 of the 3,778 translated sets carry an intensity and the rest have
  `None` — not zero, and not carried forward from a neighbouring set. The corpus
  records 2,415; the odd one out sits on the zero-rep attempt, which refuses.
- 361 sets are warm-ups.
- `rest_after` is `None` on all 3,778 translated sets. This source records none
...
clocks move together. Then run with neither `--timezone` nor
`FITNESS_TRACKER_TIMEZONE` and confirm the command refuses rather than guessing.
...
standing rather than half-replaced, which is the single transaction doing its
