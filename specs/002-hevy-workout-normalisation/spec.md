...
**Status**: Built and under review. Three questions were resolved before planning and four more during review; see "Questions resolved".
...
**Independent Test**: Derive over the 164 landed records and confirm the entity counts — 3,778 of 3,779 sets, all 1,135 exercise entries, all 334 well-formed groupings as supersets — then derive again over unchanged raw and confirm the result is identical. Delivers the domain entity that every later layer reads.
...
**Why this priority**: § 37 is not satisfied by translating what fits. One set and two groupings in the corpus do not translate, and telling apart what is wrong data from what the model has no shape for is the whole value — unavailable if the refusal is a stack trace or a dropped row.

**Independent Test**: Derive over the corpus and confirm the refusals are exactly the named ones — 1 zero-rep set, 1 non-contiguous superset, 1 single-member superset — each naming the record, the position within it, the exercise and the reason. Delivers a diagnosable account of everything raw holds that the domain will not accept.

**Acceptance Scenarios**:

1. **A missed attempt.** **Given** a set recording zero repetitions, **When** derivation runs, **Then** the set does not translate and the omission is recorded as an unmodelled case against its exercise and position. It is a real event and it is not a set, so no refinement of a rep count holds it honestly.
2. **A malformed superset.** **Given** a grouping with members either side of a non-member, or with a single member, **When** derivation runs, **Then** the grouping does not translate, the omission is recorded, and its member exercises translate as ordinary items in their recorded order — the workout is not lost to a bad grouping.
3. **An unknown set kind.** **Given** a set carrying a kind the domain does not recognise, **When** derivation runs, **Then** the set does not translate rather than defaulting to a working set.
4. **Nothing is skipped silently.** **Given** the full landed corpus, **When** derivation runs, **Then** every landing record has exactly one of three outcomes: a normalised entity, a retraction it applied, or a recorded refusal naming it. There is no fourth.
5. **Absence is absence.** **Given** a set with no recorded intensity and no recorded rest, **When** derivation runs, **Then** intensity and rest are absent — not zero, not carried forward from a neighbouring set, and not reconstructed from a linked routine (§ 11, § 37).
...
- **FR-008**: A performed exercise MUST hold a non-empty sequence of sets, and its measure — repetitions, elapsed time, or ground covered — MUST be fixed by which exercise it is, so a set and its exercise cannot disagree (§ 24). A duration recorded alongside a distance MUST NOT be carried where it is a target rather than a measurement; prescription is stored separately and never satisfies a query about what happened (§ 11).
- **FR-009**: Every set MUST carry a load. Load is relative — a signed delta against an unrecorded bodyweight — where assistance is conventionally available as well as added weight, and absolute otherwise, where the number is external load. Assistance is negative on the same axis as added weight; the source's separately-named assisted and weighted variants are translated onto one exercise, not obeyed (§ 8).
- **FR-010**: An absolute load MUST admit zero, meaning no external load. Which of the two readings an exercise takes MUST be decided by the mapping and never inferred from a value.
...
- **Load** — relative where assistance is conventionally available, absolute otherwise. Signed in the relative case, so assistance and added weight sit on one axis.
- **Measure** — repetitions, elapsed time, or ground covered. The exercise vocabulary is partitioned by it.
...
- **SC-001**: Deriving over the 164 landed records translates **3,778 of 3,779 sets**. The one that does not is the missed attempt, which is the single case the model of record identifies as a genuine gap.
- **SC-001a**: Of the corpus's 336 groupings, **334 are well-formed** and all 334 reach the output as supersets. A grouping that loses so many members to refusals that fewer than two remain is not a superset, and a single survivor translates as an ordinary item rather than being lost with the grouping.
- **SC-001b**: All **1,135 landed exercise entries** become a performed exercise. An entry that lost every set it had would yield none, because an exercise holds a non-empty sequence of sets by construction.
- **SC-002**: The only set that does not translate is the zero-rep attempt, and the only groupings that do not are one non-contiguous and one single-member. A refusal outside that set is a regression, and a case in that set silently translating is a worse one.
...
- **SC-009**: Variants differing only in how a movement is loaded resolve to one exercise — the 97 `Pull Up`, 159 `Pull Up (Assisted)` and 3 `Pull Up (Band)` sets form one series, as do the 84 `Chest Dip` and 277 `Chest Dip (Assisted)` sets, and as do a weighted crunch and a plain one — with assistance carried as negative load.
...
- **SC-011**: No set carries a value that was a target rather than a measurement. `Running`'s 19 sets keep their distance and not the interval time recorded beside it.
...
- **Band tension is recorded as given.** No scalar is the whole truth for a band, but the number written down is the operator's estimate of it and discarding it forecloses an overlay supplying a resistance range later. What stays a declared limitation is comparability: band and machine assistance cannot be told apart deterministically, so they are not one series.
...
**Answer at the time**: they translate as `Relative(0)`, with the meaning
declared rather than expressed by the value.

**Superseded during review.** With `Absolute` admitting zero (Q4), an air bike
carrying no external load needs no special case at all — `Absolute(0)` says
exactly that. What survives is the sled, which does have plates on it and does
not record them, so its nine sets understate what was moved. One declared
...
**Answer at the time**: no — ground covered and ground covered in a time are
separate measures, on the evidence that all 19 `Running` sets carry a duration
and none of the 41 carry sets does.

**Superseded during review**, by looking at the values rather than at whether
they were present. Every `Running` entry repeats one identical distance and
duration across all its sets — `400m/150s ×3`, `200m/60s ×5` — which is an
interval target, not a measurement. The clean presence/absence split was the
same fact seen from the wrong angle: the duration is present exactly where a
target was set. There are three measures, and a run is ground covered.
[Decision 0005](../../docs/decisions/0005-distance-over-time-was-prescription.md).

### Q4: What decides absolute from relative?

**Raised in review**, against the rule this spec was built on.

**Answer**: whether the load axis runs both ways. `Relative` where assistance is
conventionally available as well as added weight — a pull-up, a chin-up, a dip;
`Absolute` otherwise, where the number is external load and none of it is a real
answer. It is a convention, so it is decided per exercise in the mapping and
never inferred from a value.

The rule it replaces asked whether zero was *performable*, which made a zero on a
barbell exercise wrong by construction. That diagnosed the seven empty-bar
warm-ups correctly by accident: what makes them look wrong is that a barbell has
mass, which is a fact about the implement rather than about the axis. Reflected
in FR-009, FR-010, SC-001 and SC-002.
[Decision 0004](../../docs/decisions/0004-the-load-axis-is-bidirectional-or-it-is-not.md).

### Q5: Do banded sets refuse?

**Raised in review.** **Answer**: no. Refusing them forecloses an overlay
supplying a resistance range later, and the number written down is the
operator's estimate of the band. A banded lateral raise reads its number as
external load; a banded pull-up negates it onto the relative axis like any other
assistance.

### Q6: Which templates the operator has used for another movement

**Raised in review.** **Answer**: none of them change the mapping. A template
does not determine what was performed, so a deterministic translation should not
pretend it does. Six are recorded in the model of record under "Known aliases,
